...
function getMarks() {
  ...
  req.onreadystatechange = function(evt) {onresponse(evt,showPublicMarks);};
  req.send(null);
}
...
function showPublicMarks() {
  ...
  for (var i = 0, mark; mark = marks[i]; i++) {
    ...
    links=mark.getElementsByTagName("link"); //TODO check there is at least one
...
function getPublicTags() {
  req.open(
    "GET",
    server+"?object=TagsList ",
    true);
  req.onreadystatechange = function(evt) {onresponse(evt,showPublicTags);};
  req.send(null);
}
